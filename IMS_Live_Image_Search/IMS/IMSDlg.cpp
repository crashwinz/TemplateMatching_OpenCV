...
void CIMSDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_ALARM_FILE_PATH, m_strEditAlarmFilePath);
}
...
BEGIN_MESSAGE_MAP(CIMSDlg, CDialogEx)
	...
	ON_MESSAGE(UPDATE_TRUE_MSG, OnReceivedUpdateDataTrue)
	ON_MESSAGE(UPDATE_FALSE_MSG, OnReceivedUpdateDatafalse)


	ON_BN_CLICKED(IDC_BUTTON_SOUND_FILE_PATH_SET, &CIMSDlg::OnBnClickedButtonSoundFilePathSet)
END_MESSAGE_MAP()
...
// 	}
	...
	if(m_strTempImagePath == "")
	{
		AfxMessageBox("기준 이미지가 등록 되지 않았습니다");
		return; 
	}
	...
	if(NULL == temp)
	{
		AfxMessageBox("기준 이미지가 등록되지 않았습니다");
		return;
	}
	...
	// 상관계수를 구하여 coeff에 그려줌.
	cvMatchTemplate(img1, temp, coeff, CV_TM_CCOEFF_NORMED);
	...
	PostMessage(UPDATE_TRUE_MSG,  0, 0);

	if(dmax > m_dEditSetMatchRate)
	{
		sndPlaySound(m_strAlarmFilePath, SND_ASYNC);
	}
...
void CIMSDlg::OnBnClickedButtonThreadStart()
{
	if(m_strTempImagePath == "")
	{
		AfxMessageBox("기준이미지가 등록 되지 않아 시작할 수 없습니다");
		return;
	}
...
LRESULT CIMSDlg::OnReceivedUpdateDataTrue(WPARAM w, LPARAM l)
{
	UpdateData(true);
	return 0;
}

LRESULT CIMSDlg::OnReceivedUpdateDatafalse(WPARAM w, LPARAM l)
{
	UpdateData(false);
	return 0;
}

void CIMSDlg::OnBnClickedButtonSoundFilePathSet()
{
	CFileDialog fileDlg(TRUE, NULL, NULL, OFN_READONLY, _T("sound file(*.wav;)|*.wav|All Files(*.*)|*.*||"));

	if(fileDlg.DoModal() == IDOK)
	{
		m_strAlarmFilePath = fileDlg.GetPathName();
		SetDlgItemText(IDC_EDIT_ALARM_FILE_PATH, m_strAlarmFilePath);
	}
}
